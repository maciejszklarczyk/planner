...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: change category
      decision: "significant feature + architectural improvement + new module (Friendship)"
    - topic: insight
      decision: "scaffold-first — CRUDs built independently per domain, connecting relational layer never designed"
    - topic: primary persona scope
      decision: "regular ROLE_USER (group member) planning a trip, not admin"
    - topic: auth model change
      decision: "no change — sessions/json_login/ROLE_USER+ROLE_ADMIN stay as-is; only in-role permission boundaries change"
    - topic: new permission boundaries
      decision: "event owner/creator sees+edits own event; invited participant sees but cannot edit; group self-service deferred out of this change's scope"
    - topic: MVP flow
      decision: "user creates event solo, invites individuals directly (no group required) — group self-service deferred"
    - topic: FR-002 socratic revision
      decision: "event invitations require prior accepted friendship — not raw email invite"
    - topic: API compatibility
      decision: "project is early-stage; /events response shape (EventItemDto) MAY change freely; only UserInvitationToken onboarding flow must stay untouched"
  frs_drafted: 7
...
**Bez zmian**: model auth (sesje HTTP + `json_login`, dev: `X-Dev-User`),
role systemowe `ROLE_USER`/`ROLE_ADMIN`.

**Nowe granice uprawnień (self-service, w obrębie `ROLE_USER`) — w scope tej
zmiany (Event + Friendship)**:
- Owner/twórca eventu widzi i edytuje swój event.
- Zaproszony i zaakceptowany uczestnik eventu widzi event, ale nie edytuje.

**Odroczone poza scope tej zmiany** (patrz `## Non-Goals` i `## Open
Questions`): self-service zarządzania grupami przez zwykłego `ROLE_USER`
(create/invite/remove/role-change) — dziś nadal wyłącznie przez
`Admin\GroupMembershipController`. Istniejąca reguła
`CannotRemoveLastOwnerException` zostaje zachowana bez zmian w
międzyczasie.
...
- FR-001: User can create an event and becomes its owner. Priority: must-have. Change: modified
  > Socratic: Counter-argument considered: none raised (old Event records lack
  > an owner). Resolution: kept as written; migration/backfill strategy for
  > pre-existing events tracked in Open Questions.
- FR-002: Event owner can invite another user to the event, but ONLY if that user is already an accepted friend. Priority: must-have. Change: new
...
- FR-003: Invited user can accept or decline an event invitation; the invitation expires at the event's end date. Priority: must-have. Change: new
...
- FR-004: Event participant (accepted invitee) can view the event but cannot edit or delete it — only the owner can. Priority: must-have. Change: modified
...
- FR-005: User can send a friend request to another user. Priority: must-have. Change: new
  ...
  > Open Questions as a future NFR.
- FR-006: Invited user can accept or decline a friend request; declining starts a cooldown before the same sender can re-send to the same recipient. Priority: must-have. Change: new
  ...
  > period added after decline; exact cooldown length (days) tracked in Open
  > Questions.
- FR-007: User can view their list of friends. Priority: must-have. Change: new
...
### Group self-service (odroczone)

Świadomie odroczone poza tę zmianę — patrz `## Non-Goals` i
`## Open Questions`. Grupy self-service nie mają FR w tej rundzie.
...
- Event bez ownera (stare rekordy) nie blokuje działania API — wymaga
  strategii migracji/domyślnego ownera (patrz Open Questions)
...
- Wygasanie zaproszenia do eventu jest niezawodne: zaproszenie
  nieodpowiedziane do `endDate` eventu przestaje być akceptowalne — bez
  wyjątków i bez potrzeby ręcznej interwencji.

## Constraints & Preserved Behavior

- Flow onboardingowy oparty o `UserInvitationToken` (`/invitation/verify`,
  `/invitation/complete`) musi zostać zachowany bez zmian — zaproszenie do
  eventu dla osoby bez konta go reużywa, nie zastępuje.
- Projekt jest we wczesnej fazie — kształt odpowiedzi istniejących
  endpointów (`/events`, `EventItemDto` itp.) MOŻE się zmienić w ramach tej
  zmiany; nie jest to constraint. Frontend jest rozwijany równolegle.
- Istniejący `Admin\GroupMembershipController` (admin-only zarządzanie
  grupami) zostaje nietknięty — self-service grup jest odroczone, nie
  zastępuje admin flow.
...
- **Zapraszanie do eventu osób spoza listy znajomych** — event invite
  wymaga uprzedniej akceptowanej znajomości (FR-002 socratic resolution);
  zapraszanie po samym emailu bez relacji Friendship nie wchodzi w MVP.
- **Współwłasność/transfer ownera eventu** — jeden owner na event, bez
  mechanizmu przekazania własności czy co-ownerów.

## Forward: technical-roadmap

- Migracja/backfill dla istniejących rekordów `Event` bez pola owner —
  techniczna decyzja do podjęcia podczas implementation planning
  (`/10x-plan`), nie w PRD.
- Reużycie `UserInvitationToken` dla osób zapraszanych do eventu bez konta —
  techniczny detal integracji, do rozstrzygnięcia przy planowaniu.
