---
change_id: friendship-requests
title: User sends, accepts, and declines friend requests
status: planned
created: 2026-07-22
updated: 2026-07-22
archived_at: null
---

## Notes

Depends on `context/changes/api-exception-handling/` (roadmap Foundation
F-01), split out on 2026-07-22 during `/10x-plan` once the exception-
handling migration scope grew into a cross-cutting concern unrelated to
Friendship. Implement that change first.
